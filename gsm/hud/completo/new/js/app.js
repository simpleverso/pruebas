...
var App = (function () {
  ...
  /**
   * Temporarily set _roiRect for a flash duration then clear it.
   * Used to provide visual feedback for tap-based ROI selection.
   */
  function _flashROI(roi, durationMs) {
    _roiRect = roi;
    setTimeout(function () {
      _roiRect = null;
    }, durationMs || 200);
  ...
  function _wireROISelection() {
    ...
    // ---- Touch-based ROI selection (parallel to mouse path) ----
    // Reuses the same shared state and target creation logic.
    if (typeof TouchRegionHandler !== 'undefined') {
      TouchRegionHandler.init(_canvas, {
        getRoiMode: function () { return _roiMode; },
        setRoiMode: function (v) { _roiMode = v; },
        getRoiSelecting: function () { return _roiSelecting; },
        setRoiSelecting: function (v) { _roiSelecting = v; },
        getRoiStart: function () { return _roiStart; },
        setRoiStart: function (v) { _roiStart = v; },
        getRoiRect: function () { return _roiRect; },
        setRoiRect: function (v) { _roiRect = v; }
      }, {
        onROIFinalized: function (roi) {
          // Same manual target creation logic as the mouseup handler
          var cx = roi.x + roi.w / 2;
          var cy = roi.y + roi.h / 2;
          var isColor = (_templateMode === 'color');
          var manualId = Date.now() + Math.random();
          var manualBlob = {
            id: manualId,
            centroid: { x: cx, y: cy },
            boundingBox: { x: roi.x, y: roi.y, w: roi.w, h: roi.h },
            area: roi.w * roi.h,
            velocity: { vx: 0, vy: 0 },
            framesLost: 0,
            descriptor: new Float32Array(0),
          ...
          // Capture initial template from current frame (same as mouse path)
          var currentFrame = _displayFrame || _lastProcessedFrame;
          if (currentFrame && currentFrame.data) {
            var rx = Math.max(0, Math.round(roi.x));
            var ry = Math.max(0, Math.round(roi.y));
            var rw = Math.round(roi.w);
            var rh = Math.round(roi.h);
            if (rx + rw <= currentFrame.width && ry + rh <= currentFrame.height) {
              manualBlob._template = _extractPatch(currentFrame, rx, ry, rw, rh);
            }
          }
          ...
          // Enable auto-tracking and set PTZ to auto mode
          _autoTrackEnabled = true;
          PTZMovement.setMode('auto');
          _syncTrackingButtons();
          _updateTargetCountDisplay();

          // Exit ROI mode, reset cursor, re-enable SELECT REGION button
          _roiRect = null;
          _roiStart = null;
          _roiMode = false;
          _canvas.style.cursor = '';
        ...
        onOverlayUpdate: function (rect) {
          _roiRect = rect;
        },
        onTapFlash: function (roi) {
          _flashROI(roi, 200);
        }
      });
    }
